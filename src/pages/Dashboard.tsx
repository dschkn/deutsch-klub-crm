...
export default function Dashboard() {
  ...
  const todayTasks = allTasks.filter(t => t.status !== 'completed').slice(0, 5);
  const upcomingEvents = allEvents.filter(e => e.status === 'registration_open').slice(0, 4);
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <ComposedChart data={revenueByMonth}>
                ...
                />
                <Legend />
                <Bar yAxisId="left" dataKey="revenue" name="Выручка" fill="#3B82F6" radius={[6, 6, 0, 0]} />
                <Line yAxisId="right" type="monotone" dataKey="students" name="Клиенты" stroke="#10B981" strokeWidth={2} dot={{ fill: '#10B981', r: 4 }} />
              </ComposedChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={retentionData} layout="vertical">
                ...
                />
                <Bar dataKey="retention" fill="#3B82F6" radius={[0, 6, 6, 0]} barSize={24} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
