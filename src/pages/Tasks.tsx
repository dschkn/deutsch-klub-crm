      ...
      <Dialog open={createFor !== undefined} onOpenChange={(open) => { if (!open) setCreateFor(undefined); }}>
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-2"><Label>Связанные студенты</Label><Input value={studentQuery} onChange={event=>setStudentQuery(event.target.value)} placeholder="Начните вводить фамилию"/>{studentQuery.trim()&&<div className="max-h-44 overflow-y-auto border bg-white p-1 shadow-sm">{importedStudents.filter(student=>{const q=studentQuery.trim().toLocaleLowerCase('ru'); const name=(student.name || '').toLocaleLowerCase('ru'); return name.includes(q) || name.split(/\s+/).some(part=>part.startsWith(q));}).slice(0,20).map(student=><button type="button" key={student.id} className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-muted" onClick={()=>{setNewTask(current=>({...current,relatedStudentIds:current.relatedStudentIds.includes(student.id)?current.relatedStudentIds.filter(id=>id!==student.id):[...current.relatedStudentIds,student.id]}));setStudentQuery('')}}><Checkbox checked={newTask.relatedStudentIds.includes(student.id)}/>{student.name}</button>)}{importedStudents.filter(student=>(student.name||'').toLocaleLowerCase('ru').includes(studentQuery.trim().toLocaleLowerCase('ru'))).length===0&&<div className="px-3 py-2 text-sm text-muted-foreground">Совпадений не найдено</div>}</div>}<div className="flex flex-wrap gap-1">{newTask.relatedStudentIds.map(id=>{const student=importedStudents.find(item=>item.id===id);return student?<Badge key={id} variant="secondary" className="gap-1">{student.name}<button type="button" onClick={()=>setNewTask(current=>({...current,relatedStudentIds:current.relatedStudentIds.filter(item=>item!==id)}))}>×</button></Badge>:null})}</div></div>
          </div>
