  ...
  return <div className="space-y-4">
    ...
    <Card className="overflow-hidden border-slate-300 shadow-sm">
      ...
      <div className="overflow-x-auto bg-white"><div className="min-w-[1540px] text-[10px]">
        <div className="grid border-b-2 border-slate-500" style={{ gridTemplateColumns: '220px repeat(7, 1fr)' }}>
        ...
        {administrators.map((admin, rowIndex) => <div key={admin.id} className="grid border-b border-slate-400" style={{ gridTemplateColumns: '220px repeat(7, 1fr)' }}>
          <div className="sticky left-0 z-10 flex min-h-12 items-center gap-2 border-r-2 border-slate-500 bg-slate-100 px-2"><Avatar className="h-8 w-8"><AvatarImage src={admin.avatar}/><AvatarFallback className={cn('text-[9px] text-white', admin.accent)}>{admin.initials}</AvatarFallback></Avatar><div className="min-w-0"><p className="text-xs font-semibold">{admin.lastName} {admin.firstName}</p><p className="truncate text-[9px] text-muted-foreground">{admin.role}</p></div></div>
